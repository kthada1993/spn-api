...
const screeningSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.has_underlying_disease === 1 && !data.underlying_disease_detail) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['underlying_disease_detail'],
        message: 'underlying_disease_detail is required when has_underlying_disease is 1'
      });
    }
  });
...
export function validateScreeningInput(body) {
  ...
  return {
    ...
    underlying_disease_detail:
      data.has_underlying_disease === 1 ? data.underlying_disease_detail : null,
